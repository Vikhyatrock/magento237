define([
    ...
    else
    {
        return Component.extend({defaults:{
            ...
            var name=jQuery(event.target).attr("value");
            var fname=jQuery("#shippingFee option[value='"+name+"']").text();
            if(name>0){
            jQuery.cookie("shippingExtrafeeIds",name);
            jQuery.cookie("shippingExtraFeeLabel",fname);}
            else{
