...
use \Mageants\BannerSlider\Model\Height;
...
class Setting extends \Magento\Backend\Block\Widget\Form\Generic implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
    ...
    protected function _prepareForm()
    {
        ...
        if ($sliderid > 0) {
            $adaptive_heightDisabled =  true;
        } else {
            $adaptive_heightDisabled = false;
        }
        $main = $fieldset->addField(
            'adaptive_height',
            'select',
            [
                'name'  => 'setting[adaptive_height]',
                'label' => __('Adaptive Height'),
                'title' => __('Adaptive Height'),
                "values"    => $this->height->toOptionArray(),
                'note' => 'AdaptiveHeight True/False',
                'required' => true,
                'class' => 'validate-select',
                'disabled' => $adaptive_heightDisabled
            ]
        );
