...
class Save extends \Mageants\BannerSlider\Controller\Adminhtml\Sliders
{
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            $sliderSavedData = [];
            if ($sliderId != 0) {
                $sliderSavedData = $this->slidersFactory->create()->getCollection()->addFieldToSelect('setting')->addFieldToFilter('id', $sliderId)->load()->getData();
            }
            $remainingData = [];
            $newSliderArray = [];
            if (!empty($sliderSavedData)) {
                foreach ($sliderSavedData as $sliderSavedDataValues) {
                    foreach ($sliderSavedDataValues as $serializedSettingData) {
                        $unserializedSettingData = unserialize($serializedSettingData);
                    }
                }

                foreach ($unserializedSettingData as $seprateSettingData) {
                    foreach ($seprateSettingData as $toSeprateDataKey => $toSeprateData) {
                        if ($toSeprateDataKey == 'adaptive_height') {
                            $remainingData[$toSeprateDataKey] = $toSeprateData;
                        } elseif ($toSeprateDataKey == 'height') {
                            $remainingData[$toSeprateDataKey] = $toSeprateData;
                        }
                    }
                }
                foreach ($slidersetting as $slidersettingvalue) {
                    foreach ($slidersettingvalue as $slidersettingvaluekey => $slidersettingvalueNew) {
                        $newSliderArray[$slidersettingvaluekey] = $slidersettingvalueNew;
                    }
                    foreach ($remainingData as $remainingDatakey => $remainingDatavalue) {
                        $newSliderArray[$remainingDatakey] = $remainingDatavalue;
                    }
                }
            }
            ...
            if (!empty($newSliderArray)) {
                $slidersettingData['setting'] = $newSliderArray;
                $data['setting'] =   $this->_bannerHelper->serializeSetting($slidersettingData);
            } else {
