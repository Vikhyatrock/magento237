...
class DefaultRendererPlugin
{
    ...
    public function aroundGetColumnHtml(
        ...
        if ($column == 'custom-stock') {
            ...
            if (isset($icons['icon']) && $this->stockHelper->getDisplayOrderItemGrid()) {
                $html.="<img src={$icons['icon']} alt={$icons['label']} title='stock status' />";
                return  __($ordered_product->getResource()->getAttribute('mageants_custom_stock_status')->getFrontend()->getValue($ordered_product));
            } elseif (isset($icons['icon']) && $this->stockHelper->getDisplayOrderItemGrid()) {
                return  __($ordered_product->getResource()->getAttribute('mageants_custom_stock_status')->getFrontend()->getValue($ordered_product));
            }
