...
define([
    ...
    $.widget('mage.SwatchRenderer', {
                        ...
                        $.each(allOptions, function (key, optionObj) {
                            ...
                            for (var i = 0; i < products.length; i++) {
                                ...
                                if (simpleProductId === childProductId) {
                                   ...
                                   var select = $('div[data-attribute-id="'+ item.id +'"]').find('select');
                                   if (select.find('option').length > 0) {
                                       select.val(selectedOptionId).trigger('change');
                                   } else {
                                        var parent = $('div[data-attribute-id="'+ item.id +'"]'),
                                        label = parent.find('.' + $widget.options.classes.attributeSelectedOptionLabelClass),
                                        input = parent.find('.' + $widget.options.classes.attributeInput);
                                        parent.removeAttr('data-option-selected').find('.selected').removeClass('selected');
                                        parent.attr('data-option-selected', selectedOptionId);
                                        label.text(selectedLabel);
                                        $('input[name="super_attribute['+ item.id +']"]').val(selectedOptionId);
                                        $('.swatch-option[data-option-id='+selectedOptionId+']').addClass('selected');
                                   }
                                }
                            }
                        });
        ...
        _OnChange: function ($this, $widget) {
            ...
            if ($this.val() > 0) {
                $parent.data('option-selected', $this.val());
                $input.val($this.val());
            } else {
        ...
        _EmulateSelected: function (selectedAttributes) {
           ...
           $.each(selectedAttributes, $.proxy(function (attributeCode, optionId) {
                ...
                + '[data-attribute-code="' + attributeCode + '"] .swatch-select').val(optionId).change();

                var optionLabel =  this.element.find('.' + this.options.classes.attributeClass
                + '[data-attribute-code="' + attributeCode + '"] .swatch-select option[data-option-id="' + optionId + '"]').text();

                this.element.find('.' + this.options.classes.attributeClass
                + '[data-attribute-code="' + attributeCode + '"] .swatch-attribute-selected-option').text(optionLabel);

                this.element.find('.' + this.options.classes.attributeClass
                + '[data-attribute-code="' + attributeCode + '"] .swatch-option[data-option-id="' + optionId + '"]').trigger('click');
